...
if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		if(isset($_POST["add"])){
			...
			if($row>0){
				...
				$error_message="the car already exists";
			}
			elseif(!empty($category)){
				include_once("include/addimage.php");

				try{
					$Sql="INSERT INTO `car`( `title`, `content`, `Luggage`, `Doors`, `Passengers`, `Price`, `active`, `image`, `cat_id`) VALUES (?,?,?,?,?,?,?,?,?)";
					$stm=$conn->prepare($Sql);
					$stm->execute([$title,$content,$luggage,$doors,$passengers,$price,$active,$image_name,$category]);
					$success_message="add seccessfuly";
				}
